...
//!
//! Both the fetch and the version comparison are delegated to the single shared
//! implementation in [`crate::cli::update`] (which wraps `self_update`), so the
//! background notice and the `quantus update` command can never disagree about
//! what "latest" means.
...
/// How long we let the background refresh finish before printing the notice.
/// Bounds the wait at the end of a command so a slow network can never hold up
/// the CLI; the refresh has already been running concurrently with the command.
...
pub async fn refresh_cache_in_background() {
	...
	// `self_update` does blocking I/O, so resolve the latest version off the
...
/// Wait for the background cache refresh to finish (bounded by [`REFRESH_GRACE`]),
/// then print the update notice.
///
/// Awaiting the refresh first means the notice can appear on this same run once
/// the cache is warm, and — crucially — guarantees the notice is printed *after*
/// the command output rather than racing it into the middle. The wait is bounded
/// so a slow network can never hold up the CLI.
