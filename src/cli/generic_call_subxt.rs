...
/// SubXT-based generic call client
pub struct SubxtGenericCallClient {
    client: OnlineClient<ChainConfig>,
}

impl SubxtGenericCallClient {
    /// Create a new SubXT generic call client
    pub async fn new(node_url: &str) -> crate::error::Result<Self> {
        ...
        Ok(SubxtGenericCallClient { client })
    }
    ...
    pub async fn execute_generic_call(
        ...
        let tx_hash = match (pallet, call) {
            ...
            ("Balances", "transfer_allow_death") => {
                self.submit_balance_transfer(from_keypair, &args, false)
                    .await?
            }
            ("Balances", "transfer_keep_alive") => {
                self.submit_balance_transfer(from_keypair, &args, true)
                    .await?
            }

            // System pallet calls
            ("System", "remark") => self.submit_system_remark(from_keypair, &args).await?,

            // Sudo pallet calls
            ("Sudo", "sudo") => self.submit_sudo_call(from_keypair, &args).await?,

            // TechCollective pallet calls
            ("TechCollective", "add_member") => {
                self.submit_tech_collective_add_member(from_keypair, &args)
                    .await?
            }
            ("TechCollective", "remove_member") => {
                self.submit_tech_collective_remove_member(from_keypair, &args)
                    .await?
            }
            ("TechCollective", "vote") => {
                self.submit_tech_collective_vote(from_keypair, &args)
                    .await?
            }

            // ReversibleTransfers pallet calls
            ("ReversibleTransfers", "schedule_transfer") => {
                self.submit_reversible_transfer(from_keypair, &args).await?
            }

            // Scheduler pallet calls
            ("Scheduler", "schedule") => {
                self.submit_scheduler_schedule(from_keypair, &args).await?
            }
            ("Scheduler", "cancel") => self.submit_scheduler_cancel(from_keypair, &args).await?,
    ...
    async fn submit_balance_transfer(
        ...
        if keep_alive {
            ...
            self.submit_transaction(from_keypair, transfer_call).await
        } else {
    ...
    async fn submit_system_remark(
        ...
        let remark = args[0].as_str().ok_or_else(|| {
            QuantusError::Generic("Argument must be a string (remark)".to_string())
        })?;
        ...
        self.submit_transaction(from_keypair, remark_call).await
    }
    ...
    async fn submit_sudo_call(
        ...
        log_error!(
            "❌ Sudo calls through generic call-subxt are complex - use specific sudo wrappers"
        );
            ...
            quantus_subxt::api::tx()
                ...
                .sudo(quantus_subxt::api::Call::TechCollective(
                    quantus_subxt::api::tech_collective::Call::add_member {
                        who: subxt::ext::subxt_core::utils::MultiAddress::Id(
                            member_account_id_subxt,
                        ),
                    },
                ));

        self.submit_transaction(from_keypair, sudo_call).await
    }
    ...
    async fn submit_tech_collective_vote(
        ...
        self.submit_transaction(from_keypair, vote_call).await
    }
    ...
    async fn submit_reversible_transfer(
        ...
        self.submit_transaction(from_keypair, schedule_call).await
    }
...
pub async fn execute_generic_call_subxt(
    ...
    let call_client = SubxtGenericCallClient::new(node_url).await?;
    let keypair = crate::wallet::load_keypair_from_wallet(from, None, None)?;

    call_client
        .execute_generic_call(pallet, call, args, &keypair, tip)
        .await?;
